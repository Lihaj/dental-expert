...
class DentalExpertSystem(KnowledgeEngine):
    ...
    def _initial_action(self):
        yield Fact(action="confirm_disease")

    @Rule(Fact(action="confirm_disease"), Fact(disease=MATCH.disease))
    def ask_about_disease(self, disease):
        if disease not in self.diseases:
            print(f"Disease '{disease}' not found in our database.")
            return
        ...
        if len(confirmed_symptoms) == len(symptoms):
            ...
            if confidence >= 0.5:
                print(f"\nMay Have: High probability of {disease}")
                ...
                print(f"\nMay Have: Low probability of {disease}")
            ...
            print(f"\nVery low probability of {disease}")
    ...
    def diagnose(self):
        ...
        for symptom in {"tooth pain", "sensitivity to sweets", "visible holes", "swollen gums", "bleeding gums", "bad breath"}:
    ...
    def run(self):
        while True:
            ...
            if choice == '1':
                ...
                disease = input("Enter the disease name to confirm: ").strip()
                self.ask_about_disease(disease)
            elif choice == '3':
...
if __name__ == "__main__":
    engine = DentalExpertSystem(diseases)
    engine.reset()
    engine.run()
